...
public class DeployGithubOAuthLoginAction implements RootAction {
    ...
    public void doIndex(StaplerRequest request, StaplerResponse rsp) throws IOException, ServletException, InterruptedException {
        rsp.sendRedirect2( getDotCiConfig().getGithubWebUrl()+ "/login/oauth/authorize?client_id="
                + getClientId() + "&scope="+getScopes());
    }

    private String getClientId() {
        return DeployBoyConfiguration.get().getGithubClientID();
    }
    ...
    protected SetupConfig getDotCiConfig() {
        return SetupConfig.get();
    }


    protected DeployBoyConfiguration getDeployBoyConfig() {
    ...
    protected void updateOfflineAccessTokenForUser(String accessToken) throws IOException {
        GHUser self = GitHub.connectUsingOAuth(getDotCiConfig().getGithubApiUrl(), accessToken).getMyself();
        String login = self.getLogin();
        getDeployBoyConfig().getDeployAccessTokenRepository().updateAccessToken(login,accessToken);
    }    String getJenkinsRootUrl() {
        return Jenkins.getInstance().getRootUrl();
    }

    String postForAccessToken(String code) throws IOException {
        SetupConfig setupConfig = getDotCiConfig();
        return httpPoster.post(setupConfig.getGithubWebUrl()+"/login/oauth/access_token?" + "client_id="+getClientId() + "&"
                + "client_secret=" + getClientSecret()+ "&" + "code=" + code,new HashMap());
    }

    private String getClientSecret() {
        return DeployBoyConfiguration.get().getGithubClientSecret();
